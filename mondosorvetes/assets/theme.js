...
// CUSTOM BY SOCIAL FULL COMMERCE
...
if (windowWidth <= 500){
  ...
  $('#vitrine, .filters-toolbar').append('<p class="deslize"><-- Deslize para os lados --></p>');
}
if (windowWidth <= 500){
  function carregaDepois(){
    $('.template-product .grid--view-items').slick({
      infinite: true,
      slidesToShow: 1,
      slidesToScroll: 1,
      centerMode: true
    });
    $('.template-product .section-header').append('<p><-- Deslize para os lados --></p>');
  }
  window.onload = carregaDepois;
}

/**
 * COOKIE SOCIAL FULL COMMERCE
 */
...
// WARNING LIGHTBOX COOKIE MANAGEMENT
...
var retrieveCookie = getCookie('freteAviso');
if (!retrieveCookie) {
